...
//   // // Check if it's Safari on Mac (but not Chrome)
//   // const isMacSafari = navigator.vendor.includes('Apple') &&
//   //                     !userAgent.includes('Chrome') &&
//   //                     !isIOS;

//   // // All iOS browsers or Mac Safari should use MOV
...
function mySunFunc() {
  ...
  if (csection == 2) {
    ...
    if (is1650) {
      ...
      scale = 0.75;
      leftPosition = 50;
      sunVideo.style.width = "81%";
      if (supportsHEVCAlpha()) {
        if (!sunVideo.src.includes("sun")) {
          sunVideo.style.width = "88%";
        }
      }
    } else if (ismobile) {
      bottomPosition = -14;
